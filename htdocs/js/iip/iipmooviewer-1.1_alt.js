...
var IIP = new Class({
   ...
   */
	...
	//path controls yes or no
    this.controls = options['controls'] || null;
    this.source = main_id;
	if(!this.source)throw 'No element ID given to IIP constructor';	
	...
	if(!options['image'])throw 'Image location not set in IIP constructor options';
    ...
    window.addEvent( 'domready', function(){ 
		...
		if(height && width){
			...
			$(this.source).set('html','');
			this.load();
		}
		else{
			if(!height){
				var msg = 'height of '+main_id+' is not set ('+height+')';
				throw msg;
			}
			if(!width){
				var msg = 'width of '+main_id+' is not set ('+width+')';
				throw msg;
			}
		}
	}.bind(this) );
  ...
  createNavigationWindow: function() {	
	if(this.controls){
		...
		/*
			<img id="shiftLeft" src="images/iip/left.png"/><img id="shiftUp" src="images/iip/up.png"/><img id="shiftRight" src="images/iip/right.png"/><br/><img id="shiftDown" src="images/iip/down.png"/><br/><img id="zoomIn" src="images/iip/zoomIn.png"/><img id="zoomOut" src="images/iip/zoomOut.png"/><img id="reset" src="images/iip/reset.png"/>
		...
		*/
		...
		var navbuttons = new Element('div', {
			id: 'navbuttons',
			width: '100px',
			html: '<img id="zoomIn" src="images/iip/zoomIn.png" title="zoom in"/><img id="zoomOut" title="zoom out" src="images/iip/zoomOut.png"/><img id="reset" title="reset" src="images/iip/reset.png"/><img title="fullscreen" src="images/iip/fullscreen.png" id="fullscreen" />'
		});
		...
		$('fullscreen').addEvent('click',function(){window.open(window.location.href)});
		...
		// Add our events
		//$('#target').addEvent('mousewheel',this.zoom.bindWithEvent(this));
  ...
  refreshLoadBar: function() {
	if(this.controls){
		...
		$('loadBar').set( 'html', Math.round(this.nTilesLoaded/this.nTilesToLoad*100) + '%' );
  ...
  load: function(){
	...
	onComplete: function(transport){
		...
		if(!response){	
			var msg = "No response from server " + this.server;
			throw msg; 
		}
	    var tmp = response.split( "Max-size" );
	    if(!tmp[1]){
			var msg = "Unexpected response from server " + this.server;
			throw msg;
		}
	...
	onFailure: function(){ throw 'Unable to get image and tile sizes from server!'; }
