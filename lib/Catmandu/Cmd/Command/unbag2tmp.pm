package Catmandu::Cmd::Command::unbag2tmp;
...
sub check_orientation{
	...
	if($info->{Orientation} ne "Horizontal (normal)"){
		if($self->tempdir){
			$tmp_path = $self->tempdir."/".Data::UUID->new->create_str;
		}else{
			$tmp_path = tmpnam();
		}
...
sub create_pyramid{
	...
	if(not $success){
		$self->tee->print(" error:error while trying to create final pyramid, aborting","\n");
...
sub process_bag{
	...
	foreach my $payload(@payloads){
		...
		my $old_ac = $ma;
		$old_ac =~ s/(.+)_MA\.tif$/$1_AC.tif/i;			
		my $ac_name = $payload->name;
		$ac_name =~ s/(.+)_MA\.tif$/$1_AC.tif/i;

		my $addpath = $self->choose_path;
		my $outputdir = $self->datadir."/$addpath";
               	mkpath($outputdir);
		my $new_ac = "$outputdir/$ac_name";
		my $file_sublocation ="$addpath/$ac_name";
		$self->tee->print("MA:$ma\n");
		if(not $self->is_image($old_ac)){
			$self->tee->print("AC:$old_ac <NOT AN IMAGE>\n");
			$self->tee->print(" msg:no ac file present, so creating new one..\n");
			...
			$self->tee->print(" action:$ma -> $new_ac [creating ptif]\n");
			if(not $self->create_pyramid($ma,$new_ac)){
				$self->tee->print(" error:error while creating pyramid tiff, aborting..\n");
				return 0;
			}
			if(not $self->is_ac($new_ac)){
				$self->tee->print(" error:newly created ac $new_ac did not pass validation, aborting..\n");
			}
		}elsif(not $self->is_ac($old_ac)){
			$self->tee->print("AC:$old_ac <IMAGE, BUT NOT A PYRAMID>\n");
			$self->tee->print(" msg:this seems to be used as the resized master, so I'm basing the pyramid and the thumbs on this one..\n");
			my($changed,$tmp_path) = $self->check_orientation($old_ac);
                        if($changed){
                                $self->tee->print(" msg:orientation of MA was wrong, so creating temporary file from master..\n");
                                $self->tee->print(" tmpfile OLD_AC:$tmp_path\n");
                                $old_ac = $tmp_path;
                        }
			$self->tee->print(" action:$old_ac -> $new_ac [creating ptif]\n");
			if(not $self->create_pyramid($old_ac,$new_ac)){
				$self->tee->print(" error:error while creating pyramid tiff, aborting..\n");
                                return 0;
			}
			if(not $self->is_image($new_ac)){
                                $self->tee->print(" error:newly created ac $new_ac did not pass validation, aborting..\n");
			}
			#i.e.: gecropte versie van de master dient nu als input voor the thumbs
			$ma = $old_ac;			
		}else{
			$self->tee->print("AC:$old_ac <VALID AC>\n");
                        $self->tee->print(" msg:valid ac present in bag, just copying to new location\n");
                        $self->tee->print(" action:$old_ac -> $new_ac [copying ptif]\n");
                        my $success = copy($old_ac,$new_ac);
                        if(not $success){
                                $self->tee->print(" error:error while copying $old_ac to $new_ac, aborting..\n");
                                return 0;
                        }
		}
		my $url;
	...
	foreach my $item(@$media){
		foreach my $file(@{$item->{file}}){
			...
			my $success = $self->is_ac($file->{path});
			if(not $success){
